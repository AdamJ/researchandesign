...
const Button = ({
  ...
  external = false // If the link is external
}) => {
  const sizeMap = {
      sm: 'sm',
      small: 'sm',
  ...
  return (
    <Link to={destination}>
      <button
        className={clsx('button', sizeClass, outlineClass, variantClass, blockClass, disabledClass, className, externalClass)}
        style={style}
        role="button"
...
// Customized
{/* <Button label="Click me" link="#using" variant="secondary" size="lg" externalID="no" outline /> */}
// Disabled
